...
def mutate_tensor(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    for id,v in zip(indices,values):
      i,j=id
      x[i][j]=v
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    tmp=torch.arange(start,stop+1,dtype=torch.float64)
    x=tmp[tmp%10==0]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    tmp=x.view(2,3,4).clone()
    y=(tmp.transpose(0,1)).contiguous().view(3,8)
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    print('here is sorted x:',sorted_x)
    print('here is sorted_indices:',sorted_indices)
    mask=torch.cat((torch.tensor([True]),sorted_x[1:]!=sorted_x[:-1]))
    print('here is mask:',mask)
